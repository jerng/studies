...
## Preamble
- IMPORTANT : The later parts of this text depend on an understanding of the
  earlier parts, so the parts should be read in order ( except, you can read
  this [ Preamble ] and the [ Executive Summary ] first ).
    ...
    - Web Components ( WebComponents.Org ) UNOFFICIALLY refers to the composite
      use of a set of technologies defined in other standards documents, mainly
        ...
        - the Cascading Style Sheets (CSS) Scoping Module Level 1 W3C First
          ...
          official standard” )
- HTML documents are interpreted within the context of the Document Object Model
  (DOM).
...
- … whereby somewhat non-trivially, a “template” Element may be configured to
  replace itself with ShadowRoot, in a node tree, 
    - … by extending its (“template” Element’s) .content DocumentFragment into a
      ShadowRoot,
    - … attaching that ShadowRoot to its (“template” Element’s) parent Element
      as its shadow root, and
...
-   Nodes have “.parentNode” and “.parentElement” properties.
    -   A Node’s .parentNode points to their sole parent in their node tree.
        Thus Nodes may belong to only one tree at a time.
...
-   Certain interfaces must be at the root of their respective node trees.
    -   Documents, DocumentFragments, and ShadowRoots must have their
        ...
        have parents" ).
 -   Various other Nodes may be node roots, but the following are of special
     interest.
...
-   ShadowRoots each have a ".host" property, which must point to an Element,
    ...
    -   The inverse pointer is the Element’s “.shadowRoot” property. So,
        ...
        -   then, E.shadowRoot points to S. ( Caveat : unless S was created with
            `{ mode : "closed" }` )
...
-   A “shadow tree” refers to a node tree whose root is a ShadowRoot.
    -   WARNING : [ the node tree of a ShadowRoot ] is not [ the node tree of
        that ShadowRoot’s host ] … thus, frustratingly, “an Element’s root which
    ...
    -   From the perspective of [ the ShadowRoot and its shadow tree ], [ the
        node tree of the ShadowRoot’s host ] may be referred to as a “light
        tree”.
    ...
    -   ( The DOM spec neither recommends, nor recommends against, the use of
        ...
        DOM-spec-recommended term “light tree”. See note on “shadow DOM”. )
    -   ( The DOM spec neither recommends, nor recommends against, the use of
...
-   Node tree traversal : Node methods such as “.querySelector” do not traverse
    from an Element, to the Element’s “.shadowRoot” property’s ShadowHost ; to
    traverse the shadow tree, one has to call the respective traversal method on
    a node ( such as the ShadowRoot ) in the shadow tree.
-   Styling : Style declarations affecting a node tree, NT, do not select [
    Elements whose root is a shadow root with their shadow host in NT ]. There
    are two ways to style shadow trees.
    -   Programmatic styling : ShadowRoots have an “.adoptedStyleSheets”
        property which houses an array of CSSStyleSheet objects.
           -   Each CSSStyleSheet is treated as a style sheet that has been
            ...
            -   CSSStyleSheet objects should be created with the
    ...
    - Declarative styling : “style” Elements can be written into a shadow tree,
      which will thus be scoped to affect only its shadow tree.
	    
        -   “style” Elements can be injected into a shadow tree via “template”
            Elements.
...
-   RENDERING : Unlike other node trees whose roots are DocumentFragments,
    ...
    -   WARNING : when an Element becomes a shadow host, as soon as its
        .shadowRoot property is pointed to a shadow root, the children of shadow
        host are no longer rendered by user agents - it is as if the children
        have the CSS property “display : none” … however an examination of the
        child Elements will show that both, their initial node tree location (
        as descendants of the shadow host ), and initial value of “display”,
    ...
    -   Furthermore, a shadow host’s [ .shadowRoot property’s ShadowRoot’s ]
    ...
    -   ( The DOM spec neither recommends, nor recommends against, the use of
        ...
        elements in a shadow root’s node tree ( shadow tree ) ], from the [
        rendered elements in a shadow root’s host’s node tree ( light tree ) ].
        )
    

#### 3.2.3. Discussion of Technology Names related to Shadow Root and Shadow
Hosts
...
-   Poor analogy : Shadow trees generally do not exhibit fewer phenomena or
    ...
    -   In fact, shadow trees behave almost exactly like light trees, with few
        caveats.
...
-   Compounded poor analogy : there exists an Element, the “template” Element,
    whose “.innerHTML” markup is not rendered by the DOM, but instead parsed
    into a unrendered DocumentFragment, and stored in the “template” Element’s
    “.content” property. As if it were hidden in the shadows.
    
    -   “template” Elements do have a feature which turns them into shadow
        hosts, but this is off by default, and anyway, most shadow hosts are not
        “template” Elements.
...
-   FUNCTIONALITY : the main feature of shadow trees, is that they are SPATIALLY
    ...
    other-dimensional” ) versus their hosts’ node trees ( and perhaps should
    thus be better called “guest trees” or “protected trees” ).
    
    -   This hyperdimensionality allows shadow trees and their document trees to
        optionally maintain SPATIAL segregation ( encapsulation ) from their
        hosts’ node trees, for node traversing operations such as : event
    ...
    -   The shadow root security model is such that,
        ...
        -   [ shadow trees ] ( informally, the guest trees ) are slightly more
            protected from scripts in [ their hosts’ node trees ], whereas
		    
        -   [ node trees of shadow hosts ] are slightly less protected from
            scripts in [ their shadow roots’ node trees ] ( informally, the
            guest trees ).
		    
        -   A ShadowRoot accesses their shadow host’s node tree via the
            ShadowRoot’s .host property.
		    
        -   A shadow host accesses their descendent ShadowRoot via the shadow
            host’s .shadowRoot property, alternatively via the return value of
            the host’s .attachShadow method.
